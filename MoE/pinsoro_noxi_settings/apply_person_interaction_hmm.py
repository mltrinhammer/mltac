...
def evaluate(keys: list[Key], labels: np.ndarray, pred: np.ndarray, mode: str, param: str) -> list[dict[str, object]]:
...
def main() -> None:
    ...
    rows = evaluate(val_keys, val_labels, base_pred, "baseline", "none")
    for mix in args.transition_mixes:
        ...
        for strength in args.transition_strengths:
            ...
            rows.extend(evaluate(val_keys, val_labels, pred, "hmm_uniform_start", f"mix={mix:g};strength={strength:g};alpha={args.transition_alpha:g}"))
