"""Launch PinSoRo shared-person interaction ablations across GPUs."""
...
SCRIPT = MOE_ROOT / "pinsoro_noxi_settings" / "train_person_interaction_fusion.py"
DEFAULT_MANIFEST_ROOT = MOE_ROOT / "moe_data" / "outputs" / "windows_w2400_s1200"
DEFAULT_OUTPUT_ROOT = MOE_ROOT / "experiments" / "pinsoro_person_interaction_early_fusion"
...
class RunSpec:
    ...
    def run_name(self) -> str:
        return (
            ...
            f"shared_person_{self.interaction_mode}_seed13"
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PinSoRo person-interaction early-fusion ablations.")
    parser.add_argument("--python", type=Path, default=PROJECT_ROOT / ".venv-gpu/bin/python")
    parser.add_argument("--gpus", default="0,1,2")
    parser.add_argument("--domain-scopes", nargs="+", choices=("both", "CC", "CR"), default=["CC", "CR"])
    parser.add_argument("--interaction-modes", nargs="+", choices=("none", "linear", "tcn"), default=["none", "linear", "tcn"])
...
def main() -> None:
    ...
    for scope in args.domain_scopes:
        for mode in args.interaction_modes:
            run_queue.put(RunSpec(scope, mode))
