"""Diagnose whether CR social class 3 is separable from frozen MoE expert logits.

This script trains simple binary classifiers on train-internal CR social rows:
class 3 vs not class 3. It evaluates on validation and writes threshold,
...
DEFAULT_OUTPUT = EXPERIMENT_ROOT / "moe1_cr_social_class3_binary_diagnostic"
Key = tuple[str, str, str, str, str, int]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Binary class-3 diagnostic from frozen CR social expert logits.")
...
def session_rows(keys: list[Key], y_true: np.ndarray, score: np.ndarray, threshold: float) -> list[dict[str, object]]:
    ...
    for session in sessions:
        ...
        rows.append({
            "session_id": session,
            "n_frames": len(idx),
            "class3_support": support,
            "predicted_class3": predicted,
            "precision": tp / predicted if predicted else float("nan"),
            "recall": tp / support if support else float("nan"),
            "tp": tp,
            "fp": fp,
            "fn": fn,
            "mean_score_true3": float(score[idx][y_true[idx] == 1].mean()) if support else float("nan"),
            "mean_score_not3": float(score[idx][y_true[idx] == 0].mean()) if support < len(idx) else float("nan"),
        })
    return rows
...
def distribution_rows(y_true: np.ndarray, score: np.ndarray) -> list[dict[str, object]]:
    rows = []
    for label, mask in (("true3", y_true == 1), ("not3", y_true == 0)):
        values = score[mask]
        rows.append({
            "group": label,
            "n": len(values),
...
def run_one(args: argparse.Namespace, feature_kind: str, class_weight: str) -> dict[str, object]:
    ...
    train_y = (train_multi == 3).astype(np.int64)
    val_y = (val_multi == 3).astype(np.int64)
