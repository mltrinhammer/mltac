"""Train MoE 1 modality experts for one PinSoRo domain and export train logits."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run MoE 1 dyadic TCN experts for CC or CR across available GPUs."
    )
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.root is None:
        args.root = EXPERIMENT_ROOT / f"moe1_{args.domain.lower()}_experts"
    return args
...
def eval_command(args: argparse.Namespace, expert: Expert) -> list[str]:
    # evaluate_pinsoro_checkpoint reads mmap_cache_root from config.json.
    return [
        str(args.python),
        str(PROJECT_ROOT / "scripts/evaluate_pinsoro_checkpoint.py"),
        "--run-dir",
...
def run_expert(args: argparse.Namespace, expert: Expert, gpu: str) -> None:
    ...
    with log_path.open("a", encoding="utf-8") as log:
        ...
        (current / COMPLETE_MARKER).write_text(
            "training and train-logit export complete\n", encoding="utf-8"
        )
