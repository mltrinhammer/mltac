"""Train NOXI/NOXI-J metadata-head MoE modality experts and export train predictions."""
...
class Expert:
    ...
    def metadata(self) -> Path:
        return MOE_ROOT / self.corpus.data_dir / "outputs" / "metadata" / "role_metadata.csv"

    @property
    def run_name(self) -> str:
        return f"{self.corpus.name}_{self.feature}_dyadic_tcn_k11_metadata_head_seed13"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run NOXI metadata-head dyadic TCN experts across GPUs.")
...
def normalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.root is None:
        args.root = EXPERIMENT_ROOT / f"noxi_moe1_{args.corpus}_metadata_head_experts"
    return args
...
def train_command(args: argparse.Namespace, expert: Expert) -> list[str]:
    return [
        str(args.python),
        str(MOE_ROOT / "train_noxi_metadata_head_tcn.py"),
        "--manifest",
        str(expert.manifest),
        "--metadata",
        str(expert.metadata),
...
def eval_command(args: argparse.Namespace, expert: Expert) -> list[str]:
    return [
        str(args.python),
        str(MOE_ROOT / "evaluate_noxi_metadata_head_checkpoint.py"),
        "--run-dir",
...
def run_expert(args: argparse.Namespace, expert: Expert, gpu: str) -> None:
    ...
    with log_path.open("a", encoding="utf-8") as log:
        ...
        (current / COMPLETE_MARKER).write_text("metadata-head training and train-prediction export complete\n", encoding="utf-8")
...
def main() -> None:
    ...
    experts = tuple(Expert(args.corpus_config, feature, args.window_size, args.stride) for feature in args.features)
    for expert in experts:
        if not expert.manifest.is_file():
            raise FileNotFoundError(expert.manifest)
        if not expert.metadata.is_file():
            raise FileNotFoundError(f"{expert.metadata} (run ACM/MoE/prepare_noxi_metadata.py first)")
