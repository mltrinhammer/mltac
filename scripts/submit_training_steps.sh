...
preprocess_cmd=(
    sbatch
    --parsable
    --export "ALL,INCLUDE_MULTIMODAL_ABLATION=$([ -n "${SELECTED_STEP_MAP[4]+x}" ] && echo 1 || echo 0),INCLUDE_WINDOW_ABLATION=$([ -n "${SELECTED_STEP_MAP[5]+x}" ] && echo 1 || echo 0),INCLUDE_EXPANDED_MULTIMODAL=${INCLUDE_EXPANDED_MULTIMODAL:-0}"
