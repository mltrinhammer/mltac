...
# Partner-lag TCN trainer.
#
# This experiment uses dyadic tensors, but it does not feed the concatenated
# dyad into one shared TCN. Instead, novice and expert streams are split,
# encoded by separate TCNs, and then each role-specific head receives its own
# hidden sequence plus lagged hidden states from the partner.
...
from src.acm_pipeline.models_tcn import PartnerLagTCNRegressor
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a partner-lag TCN from a dyadic manifest.")
    ...
    parser.add_argument("--partner-lags", type=int, nargs="+", default=[0], help="Partner hidden-state lags in frames; -25 means partner t-25, +25 means partner t+25.")
...
def make_run_dir(args: argparse.Namespace) -> Path:
    run_name = args.run_name.strip() or f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_tcn_partner_lag"
...
def train_one_epoch(model: PartnerLagTCNRegressor, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device, ccc_weight: float) -> float:
    model.train()
    losses = []
    for batch in loader:
        x = batch["x"].to(device)
        ...
        # The partner-lag model still predicts both role channels at every
        # frame, so the same dyadic masked losses are valid here.
...
@torch.no_grad()
def reconstruct_validation(model: PartnerLagTCNRegressor, dataset: WindowedDyadicDataset, loader: DataLoader, device: torch.device) -> list[dict[str, object]]:
    model.eval()
    ...
    # Window predictions are averaged back onto the full session timeline. This
    # makes validation metrics comparable with the other dyadic TCN runs.
    for batch in loader:
...
def main() -> None:
    ...
    # This model is intentionally less symmetric than DyadicTCNRegressor:
    # each role has a separate encoder and separate head. Partner information
    # enters only through shifted hidden states at the head.
    model = PartnerLagTCNRegressor(
        n_features_per_role=n_features_per_role,
        hidden_channels=args.hidden_channels,
        levels=args.levels,
        kernel_size=args.kernel_size,
        dropout=args.dropout,
        partner_lags=tuple(args.partner_lags),
    ).to(device)
