...
# Transformer training uses the same transformed-manifest contract as the TCN.
# The difference is entirely model-side: frame features are projected to
# d_model, positional encoding is added, and encoder self-attention models the
# temporal window.
...
from src.acm_pipeline.models_transformer import TransformerRegressor
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an encoder-only Transformer baseline from any transformed feature manifest.")
...
def make_run_dir(args: argparse.Namespace) -> Path:
    # Use explicit run names for real experiments. Timestamped names are mainly
    # useful for smoke tests and exploratory local checks.
    if args.run_name.strip():
        run_name = args.run_name.strip()
    else:
        run_name = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_transformer"
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        # frame_mask is passed both to attention, to hide padding, and to the
        # loss through loss_mask, to hide missing targets.
        pred = model(x, frame_mask=frame_mask)
...
def reconstruct_validation(
    ...
    # As with the TCN, validation uses overlapping windows. Reconstruct session
    # predictions by averaging all window predictions that cover each frame.
    for batch in loader:
        x = batch["x"].to(device)
        frame_mask_t = batch["frame_mask"].to(device)
        pred = model(x, frame_mask=frame_mask_t).detach().cpu().numpy()
...
def main() -> None:
    ...
    model = TransformerRegressor(
        ...
        max_len=max(args.window_size, 1),
    ).to(device)
