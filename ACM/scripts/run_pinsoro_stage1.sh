...
WINDOW_STRIDE="${WINDOW_STRIDE:-62}"
...
for feature_set in "${FEATURE_SETS[@]}"; do
    aligned="${ACM_DIR}/outputs/pinsoro/manifests/${feature_set}_25hz.csv"
    normalized="${ACM_DIR}/outputs/pinsoro/manifests/${feature_set}_raw.csv"
    "${PYTHON_BIN}" "${SCRIPT_DIR}/pinsoro_prepare_feature_tensors_25hz.py" --feature-set "${feature_set}"
