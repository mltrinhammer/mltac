...
# Dyadic Transformer trainer. Same encoder-only architecture as the role-level
# Transformer, but the regression head predicts two channels per frame.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a dyadic encoder-only Transformer baseline.")
...
def make_run_dir(args: argparse.Namespace) -> Path:
    run_name = args.run_name.strip() or f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_transformer_dyadic"
...
def train_one_epoch(model: TransformerRegressor, loader: DataLoader, optimizer: torch.optim.Optimizer, device: torch.device, ccc_weight: float) -> float:
    model.train()
    losses = []
    for batch in loader:
        x = batch["x"].to(device)
...
@torch.no_grad()
def reconstruct_validation(model: TransformerRegressor, dataset: WindowedDyadicDataset, loader: DataLoader, device: torch.device) -> list[dict[str, object]]:
    model.eval()
