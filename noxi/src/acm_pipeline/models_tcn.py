...
class ChannelLayerNorm(nn.Module):
    """Normalize channels independently at each frame without temporal leakage."""
...
class TemporalBlock(nn.Module):
    """Residual dilated Conv1d block with optional causal padding."""
    ...
    def __init__(
        ...
        super().__init__()
        symmetric_padding = dilation * (kernel_size - 1) // 2
        causal_padding = dilation * (kernel_size - 1)

        def convolution(in_features: int, out_features: int) -> nn.Module:
            layer = nn.Conv1d(
                in_features,
                out_features,
                kernel_size,
            ...
            if not causal:
                return layer
            return nn.Sequential(nn.ConstantPad1d((causal_padding, 0), 0.0), layer)

        # Causal blocks normalize channels independently per frame; GroupNorm
        # would otherwise leak future-frame statistics across the time axis.
        normalization = ChannelLayerNorm if causal else lambda channels: nn.GroupNorm(1, channels)
...
class MetadataConditionedIndependentDyadicTCNRegressor(nn.Module):
    """Independent person TCN with metadata-conditioned output calibration."""
    ...
    def __init__(
        ...
        super().__init__()
        if metadata_injection != "output_calibration":
            raise ValueError("The simple backbone supports only output_calibration metadata injection.")
        if metadata_dim <= 0:
            raise ValueError("metadata_dim must be positive for metadata-conditioned models.")

        self.metadata_dim = metadata_dim
        self.base = IndependentDyadicTCNRegressor(
        ...
        self.metadata_encoder = nn.Sequential(
            nn.Linear(2 * metadata_dim, metadata_embedding_dim),
            nn.ReLU(),
            nn.Dropout(metadata_dropout),
        )
        self.calibration = nn.Linear(metadata_embedding_dim, 4)
        nn.init.zeros_(self.calibration.weight)
        nn.init.zeros_(self.calibration.bias)

    def _metadata_embedding(self, metadata: torch.Tensor) -> torch.Tensor:
        if metadata.ndim != 3 or metadata.shape[1] != 2 or metadata.shape[2] != self.metadata_dim:
            raise ValueError(
                f"metadata must have shape [batch, 2, {self.metadata_dim}], got {tuple(metadata.shape)}"
            )
        return self.metadata_encoder(metadata.reshape(metadata.shape[0], -1))

    def forward(self, x: torch.Tensor, metadata: torch.Tensor | None = None) -> torch.Tensor:
        if metadata is None:
            raise ValueError("metadata is required for metadata-conditioned independent TCN.")
        raw = self.base(x)
        affine = self.calibration(self._metadata_embedding(metadata))
        scale = 1.0 + 0.1 * torch.tanh(affine[:, :2]).unsqueeze(1)
        bias = affine[:, 2:].unsqueeze(1)
...
class MetadataConditionedDyadicTCNRegressor(nn.Module):
    """Shared dyadic TCN with conservative metadata-conditioned readout options."""
    ...
    def __init__(
        ...
        super().__init__()
        if metadata_injection not in {"output_calibration", "before_head"}:
            raise ValueError(f"Unsupported metadata_injection: {metadata_injection}")
        if metadata_dim <= 0:
            raise ValueError("metadata_dim must be positive for metadata-conditioned models.")

        self.metadata_injection = metadata_injection
        self.metadata_dim = metadata_dim
        channels = [input_dim] + [hidden_channels] * levels
        ...
        if metadata_injection == "before_head":
            self.head = nn.Conv1d(hidden_channels + metadata_embedding_dim, 2, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor, metadata: torch.Tensor | None = None) -> torch.Tensor:
        if metadata is None:
            raise ValueError("metadata is required for metadata-conditioned dyadic TCN.")
        hidden = self.tcn(x)
        meta = self._metadata_embedding(metadata)
        if self.metadata_injection == "before_head":
            expanded = meta.unsqueeze(-1).expand(-1, -1, hidden.shape[-1])
            return self.head(torch.cat([hidden, expanded], dim=1)).transpose(1, 2)

        raw = self.head(hidden).transpose(1, 2)
        assert self.calibration is not None
        affine = self.calibration(meta)
        scale = 1.0 + 0.1 * torch.tanh(affine[:, :2]).unsqueeze(1)
        bias = affine[:, 2:].unsqueeze(1)
        return scale * raw + bias


class MetadataOutputCalibratedRegressor(nn.Module):
    """Apply metadata-conditioned output calibration to any dyadic regressor."""
...
class MultimodalTurnTCNRegressor(nn.Module):
    ...
    def __init__(
        ...
        if metadata_injection != "none" and metadata_injection != "output_calibration" and backbone_model != "dyadic_shared":
            raise ValueError("Only the dyadic_shared backbone supports metadata injection modes other than output_calibration.")
        if metadata_injection == "before_head" and backbone_model == "simple":
            raise ValueError("The simple backbone supports only output_calibration metadata injection.")
        if metadata_injection == "none" and metadata_dim != 0:
            raise ValueError("metadata_dim must be 0 when metadata_injection='none'.")
        ...
        def calibrate_if_needed(base: nn.Module) -> nn.Module:
            if metadata_injection == "none":
                return base
            if metadata_injection != "output_calibration":
                raise ValueError(f"Unsupported metadata_injection for {backbone_model}: {metadata_injection}")
            return MetadataOutputCalibratedRegressor(
                base=base,
        ...
        if backbone_model == "simple":
            if metadata_injection == "none":
                self.backbone: nn.Module = IndependentDyadicTCNRegressor(**common, causal_tcn=causal_tcn)
            else:
                self.backbone = MetadataConditionedIndependentDyadicTCNRegressor(
                    n_features_per_role=fused_features_per_role,
                    metadata_dim=metadata_dim,
            ...
            if metadata_injection == "none":
                ...
                self.backbone = MetadataConditionedDyadicTCNRegressor(
                    input_dim=2 * fused_features_per_role,
                    metadata_dim=metadata_dim,
                    metadata_injection=metadata_injection,
                    metadata_embedding_dim=metadata_embedding_dim,
            ...
            base = DyadicRoleHeadTCNRegressor(
                input_dim=2 * fused_features_per_role,
                hidden_channels=hidden_channels,
                levels=levels,
                kernel_size=kernel_size,
            ...
            self.backbone = calibrate_if_needed(base)
        elif backbone_model == "gated_partner":
            base = GatedPartnerTCNRegressor(**common, causal_tcn=causal_tcn)
            self.backbone = calibrate_if_needed(base)
        elif backbone_model == "shared_attention":
            base = SharedEncoderRoleAttentionTCNRegressor(
                **common,
            ...
            base = RoleAttentionTCNRegressor(
                **common,
                attention_context=attention_context,
                attention_heads=attention_heads,
                attention_past_frames=attention_past_frames,
    ...
    def forward(
        ...
        if self.metadata_injection == "none":
            pred = self.backbone(fused_x)
        else:
            pred = self.backbone(fused_x, metadata=metadata)
