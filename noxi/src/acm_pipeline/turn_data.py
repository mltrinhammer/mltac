...
class TurnDataset(Dataset):
    ...
    def preload(self) -> None:
        """Load every backing tensor once into this dataset object's cache."""

        for turn in self.turns:
            self._load(turn.novice_example)
            self._load(turn.expert_example)
    ...
    def preload(self) -> None:
        ...
        for turn in self.turns:
            for modality_name in turn.modality_order:
                self._load(turn.novice_examples[modality_name])
                self._load(turn.expert_examples[modality_name])
