...
def load_session_tensor(example: ManifestExample) -> SessionTensor:
    ...
    try:
        size_mb = example.tensor_path.stat().st_size / 1_000_000
    except OSError:
        size_mb = float("nan")
    print(f"tensor_load_start key={example.key} size_mb={size_mb:.1f} path={example.tensor_path}", flush=True)
    ...
    print(f"tensor_load_done key={example.key} shape={tuple(x.shape)} seconds={elapsed:.2f}", flush=True)
    return SessionTensor(x=x, y=y, target_mask=target_mask)
