...
RUNS = [
    ...
    {
        'name': 'cc_social_submitted_head_adapter_bias',
        'head': 'social',
        'run_dir': 'MoE/experiments/pinsoro_head_architectures_temporal_delta010_metadata/pinsoro_cc_headarch_head_adapters_delta010_metadata_seed13',
        'logit_bias': [0.0, 0.0, 1.0, -1.5, 0.75],
    },
]
...
def apply_logit_bias(logits: np.ndarray, head: str, bias: list[float] | None) -> np.ndarray:
    if not bias:
        return logits
    n = hmm.CLASS_COUNTS[head]
    if len(bias) != n:
        raise ValueError(f'Bias length {len(bias)} does not match {head} class count {n}')
    adjusted = logits.copy()
    adjusted[:, :n] += np.asarray(bias, dtype=np.float64)
    return adjusted
...
def main():
    ...
    ap.add_argument('--output-root', type=Path, default=MOE_ROOT / 'experiments' / 'cc_annotator_sensitivity_submitted_social_2906')
    ...
    for spec in RUNS:
        ...
        logits = apply_logit_bias(logits, head, spec.get('logit_bias'))
