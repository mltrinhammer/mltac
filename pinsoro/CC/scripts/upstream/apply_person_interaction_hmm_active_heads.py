...
def write_hmm_submission_tree(output_dir: Path, keys: list[Key], pred: np.ndarray) -> None:
    """Write organizer-format predictions for the active smoothed heads only."""
    ...
    for (domain, session_id, role, head), values in sorted(grouped.items()):
        values.sort()
        frame_indices = [frame_idx for frame_idx, _value in values]
        expected = list(range(len(values)))
        if frame_indices != expected:
            raise RuntimeError(
                "HMM submission export requires full contiguous frame coverage for "
                f"{domain}/{session_id}/{role}/{head}; "
                f"got first={frame_indices[:3]} last={frame_indices[-3:]} n={len(frame_indices)}."
            )
        ...
        with path.open("w", encoding="utf-8", newline="") as handle:
            ...
            for _frame_idx, prediction in values:
                if prediction < 0 or prediction >= CLASS_COUNTS[head]:
                    raise ValueError(
                        f"Invalid HMM {head} prediction {prediction} for "
                        f"{domain}/{session_id}/{role}."
                    )
...
def write_smoothing_summary(output_dir: Path, rows: list[dict[str, object]], best: dict[str, object]) -> None:
    baseline = [row for row in rows if row["mode"] == "baseline" and row["head"] == "mean"]
    summary = {
        "raw_baseline": baseline[0] if baseline else None,
        "best_hmm": best,
        "selection_metric": "mean kappa over active heads",
        "val_hmm_results": "val_hmm_results.csv",
    }
    (output_dir / "smoothing_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    write_smoothing_summary(args.output_dir, rows, best)
    ...
    if args.write_test:
        ...
        write_hmm_submission_tree(args.output_dir / "test_submission_format", test_keys, test_pred)
