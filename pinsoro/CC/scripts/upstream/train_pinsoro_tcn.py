...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--causal-attention", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--domain-social-heads", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument(
        "--cr-social-weighting",
...
def compute_cr_social_weights(
    ...
    class2_weight: float,
    class3_weight: float,
    class0_weight: float = 1.0,
) -> torch.Tensor | None:
    if mode == "shared_inverse":
        return None
    counts = np.zeros(CLASS_COUNTS["social"], dtype=np.int64)
    seen: set[Path] = set()
    for window in windows:
        if window.domain != "CR":
            continue
        for role_idx, paths in enumerate(window.tensor_paths):
            ...
            seen.add(label_path)
            data = dataset.load_full_role(window, role_idx)
            labels = np.asarray(data["social_y"], dtype=np.int64)
            mask = np.asarray(data["social_mask"]).astype(bool)
            counts += np.bincount(labels[mask], minlength=CLASS_COUNTS["social"])

    present = counts > 0
    weights = np.zeros(CLASS_COUNTS["social"], dtype=np.float32)
    if mode == "unweighted":
        weights[present] = 1.0
    elif mode in {"sqrt_inverse", "capped_inverse"}:
        inverse = np.zeros_like(weights)
        inverse[present] = counts.sum() / (present.sum() * counts[present])
        weights = np.sqrt(inverse) if mode == "sqrt_inverse" else np.minimum(inverse, cap)
    elif mode == "targeted":
        weights[present] = 1.0
        weights[0] = class0_weight if present[0] else 0.0
        weights[2] = class2_weight if present[2] else 0.0
        weights[3] = class3_weight if present[3] else 0.0
    else:
        raise ValueError(f"Unknown CR-social weighting mode: {mode}")
    if not np.any(weights > 0):
        raise RuntimeError("No supervised CR-social labels found for CR-specific weighting.")
    # Keep CR-social's average frame weight at one so this changes class balance,
    # not the overall CR-versus-CC contribution to the shared social loss.
    weights *= counts.sum() / np.dot(counts, weights)
...
def reconstruct(
                ...
                if key not in accumulators:
                    ...
                    accumulators[key] = {
                        ...
                        "coverage_count": np.zeros(
                            window.session_aligned_len, dtype=np.int32
                        ),
                        "task_sum": np.zeros(
    ...
    for acc in accumulators.values():
        ...
        for head in HEADS:
            ...
            shifted = averaged_logits[covered] - averaged_logits[covered].max(
...
def main() -> None:
    ...
    model = build_pinsoro_tcn(
        ...
        args.causal_tcn,
        args.causal_attention,
        args.domain_social_heads,
    ).to(device)
    ...
    if checkpoint_path.exists():
        ...
        if test_windows:
            ...
            write_prediction_scores(
                run_dir / "test_prediction_scores.csv.gz",
                test_reconstructed,
