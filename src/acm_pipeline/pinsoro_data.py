...
class PinSoRoWindowDataset(Dataset):
                ...
                loaded.update(
                    {
                        ...
                        for key in OPTIONAL_TENSOR_ARRAY_KEYS
            ...
            loaded.update(
                {
                    key: np.load(cache_dir / f"{key}.npy", mmap_mode="r")
                    for key in OPTIONAL_TENSOR_ARRAY_KEYS
                    if (cache_dir / f"{key}.npy").is_file()
                }
            )
    ...
    def __getitem__(self, idx: int) -> dict[str, object]:
        ...
        item = {
            ...
            "domain_id": DOMAIN_IDS[window.domain],
            "window_index": idx,
        }
        if all("task_soft_y" in data and "social_soft_y" in data for data in label_data):
            item["task_soft_y"] = np.stack([data["task_soft_y"][s:e] for data in label_data], axis=0)
            item["social_soft_y"] = np.stack([data["social_soft_y"][s:e] for data in label_data], axis=0)
            item["task_soft_mask"] = (
                np.stack([data["task_soft_mask"][s:e] for data in label_data], axis=0)
                * supervised
            )
            item["social_soft_mask"] = (
                np.stack([data["social_soft_mask"][s:e] for data in label_data], axis=0)
                * supervised
            )
            item["task_weight"] = np.stack([data["task_weight"][s:e] for data in label_data], axis=0)
            item["social_weight"] = np.stack([data["social_weight"][s:e] for data in label_data], axis=0)
        return item
...
def pinsoro_window_collate(batch: list[dict[str, object]]) -> dict[str, object]:
    ...
    required_keys = ("x", "task_y", "social_y", "task_mask", "social_mask")
    optional_keys = tuple(
        key for key in OPTIONAL_TENSOR_ARRAY_KEYS if all(key in item for item in batch)
    )
    arrays = {
        key: np.stack([item[key] for item in batch])
        for key in required_keys + optional_keys
    }
    out = {key: torch.from_numpy(value) for key, value in arrays.items()} | {
        "domain_id": torch.as_tensor(
            [item["domain_id"] for item in batch], dtype=torch.long
        ),
    ...
    if "metadata" in batch[0]:
        out["metadata"] = torch.from_numpy(np.stack([item["metadata"] for item in batch]).astype(np.float32, copy=False))
    return out
