...
NOXI_ROOT="MoE/noxi_joint_settings/experiments_group_meanpool_temporal_loss"
...
run_noxi_temporal() {
  ...
  local run_name="${dataset}_audio_text_visual_w500_s125_postpred_linear_shared_encoder_temporal_mse020_delta010_seed13"
  ...
  if [[ ! -f "$run_dir/model_best.pt" ]]; then
    run_logged "$NOXI_ROOT/logs/${run_name}.log"       "$PYTHON_BIN" scripts/train_mpii_group_meanpool_multimodal_temporal.py       --manifest "$manifest"       --output-root "$NOXI_ROOT"       --run-name "$run_name"       --train-split train_internal       --val-split val_internal       --test-splits test_internal test_additional       --fusion-mode gated       --fusion-channels 64       --modality-dropout 0.1       --hidden-channels 64       --levels 4       --kernel-size 5       --dropout 0.2       --encoder-sharing shared       --prediction-interaction-scale 0.1       --batch-size 32       --epochs 30       --patience 6       --min-epochs 5       --min-delta 0.001       --lr 1e-3       --weight-decay 1e-4       --mse-weight 0.20       --ccc-weight 1.0       --delta-mse-weight 0.10       --num-workers 2       --seed 13       --device cuda
  else
...
run_noxi_temporal noxi
run_noxi_temporal noxij
run_pinsoro_temporal CC
run_pinsoro_temporal CR

echo "temporal-loss queue complete: $(date '+%Y-%m-%d %H:%M:%S')"
